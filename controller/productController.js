...
getProduct: async (req, res) => {
    try {
        ...
        const products = await productModel.aggregate([{$lookup:{from:"categories",localField:"categoryName",foreignField:"_id",as:"categoryinfo"}},{$unwind:"$categoryinfo"}])
   ...
   getAddproducts : async(req,res)=>{
    try{
        ...
        res.render("admin/addproduct",{category :categories,error:req.flash("error")})
    }catch(error){
   ...
   getEditProduct : async(req,res)=>{
    try{
        ...
        const product = await productModel.aggregate([{$match:{_id:new ObjectId(req.params.id)}},{$lookup:{from:"categories",localField:"categoryName",foreignField:"_id",as:"categoryinfo"}},{$unwind:"$categoryinfo"}])
        console.log(product[0])
        res.render("admin/updateproduct",{product : product[0],category:categories})
   ...
   postEditProduct : async(req,res)=>{
    try{
        ...
        const {productName,categoryName,price,stock,size,usertype,description,}=req.body;
